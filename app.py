...
if not st.session_state["lgpd_aceito"]:
    ...
    <div style="position:fixed;bottom:0;left:0;right:0;z-index:9999;
      background:rgba(5,8,14,0.97);border-top:1px solid rgba(0,200,255,0.18);
      padding:1rem 2rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:1rem;">
      <div style="flex:1;min-width:260px;">
        <p style="color:white;font-size:.82rem;font-weight:600;">🍪 Privacidade & Cookies — LGPD</p>
        <p style="color:#5a7a9a;font-size:.72rem;">Utilizamos cookies em conformidade com a LGPD (Lei 13.709/2018).</p>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        if st.button("Recusar"):
            st.warning("Aceite os termos para continuar.")
    with col3:
...
if not st.session_state["autenticado"]:
    ...
    with col2:
        ...
        with st.form("login_form"):
            st.markdown('<p style="color:#5a7a9a;font-size:.7rem;">Usuário</p>', unsafe_allow_html=True)
            usuario_input = st.text_input("u", placeholder="admin, analista, nubank...", label_visibility="collapsed")
            st.markdown('<p style="color:#5a7a9a;font-size:.7rem;">Senha</p>', unsafe_allow_html=True)
        ...
        st.markdown("""
        <div style="margin-top:1rem;background:rgba(0,200,255,0.02);border-radius:14px;padding:1rem;">
          <p style="color:#3a5a7a;font-size:.6rem;">Acessos de demonstração</p>
          <p style="color:#6a8aaa;font-size:.7rem;font-family:monospace;">admin/admin123 | analista/analista123 | nubank/nubank123</p>
        </div>
        """, unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(f"""
    <div style="background:rgba(0,200,255,.03);border-radius:12px;padding:.7rem;">
      <p style="color:#2a4060;font-size:.52rem;">PERFIL</p>
      <p style="color:white;font-weight:700;">{perfil_atual['perfil']}</p>
      <p style="color:#4a6a8a;font-size:.62rem;">@{usuario_atual}</p>
    </div>
    """, unsafe_allow_html=True)
    badge_db = '<span class="badge-db">📁 SQLite Ativo</span>' if sqlite_ativo else '<span style="color:#ff4444;">⚠️ Offline</span>'
    ...
    st.markdown("---")
    st.markdown(f"""
    <div style="background:rgba(0,200,255,.03);border-radius:10px;padding:.7rem;text-align:center;">
      <p style="color:#2a4060;font-size:.52rem;">Acurácia do Modelo</p>
      <p style="color:#00d4ff;font-size:1.35rem;font-weight:700;">{acuracia:.1%}</p>
    </div>
    """, unsafe_allow_html=True)
    st.markdown("---")
...
<div class="sentinel-header">
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
    <div>
      <p style="color:#2a4060;font-size:.55rem;">{f"Cliente: {cliente_vinculado}" if cliente_vinculado else "Visão Global"}</p>
      <h1 style="color:white;margin:0;font-size:1.2rem;">Painel de Segurança Cibernética</h1>
    </div>
    <div style="text-align:right;">
      <span class="badge-online">● PROTEGIDO</span>
      <p style="color:#2a4060;font-size:.55rem;">{now_str}</p>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    if not perfil_atual["pode_analisar"]:
        st.warning("⚠️ Sem permissão para análises.")
    else:
        ...
        ca, cb = st.columns(2)
        with ca:
            tipo_incidente = st.selectbox("Tipo de incidente", tipos)
            origem_ataque = st.selectbox("Origem do ataque", origens)
            cliente_afetado = st.selectbox("Cliente afetado", clientes)
        with cb:
            tempo_resolucao = st.slider("Tempo de resolução (min)", 1, 120, 30)
            status_atual = st.selectbox("Status atual", status_opts)
        ...
        if st.button("🚀 Executar análise", use_container_width=True):
            ...
            if resultado == "crítica":
                st.error(f"🔴 Severidade: CRÍTICA — AÇÃO IMEDIATA")
            elif resultado == "média":
                st.warning(f"🟡 Severidade: MÉDIA — MONITORAR")
            else:
                st.success(f"🟢 Severidade: BAIXA — CONTROLADO")
...
with tab2:
    ...
    fig = px.area(df_time, x="DATA", y="Incidentes", title="Volume ao Longo do Tempo", color_discrete_sequence=["#00d4ff"])
    fig.update_layout(**L)
    st.plotly_chart(fig, use_container_width=True)

    g3, g4 = st.columns(2)
    with g3:
        fig = px.histogram(df_vis, x="PAIS_ATAQUE", title="Ataques por País", color_discrete_sequence=["#f59e0b"])
        fig.update_layout(**L)
        st.plotly_chart(fig, use_container_width=True)
    with g4:
        dmg = df_vis.groupby("CLIENTE")["PREJUIZO_ESTIMADO"].sum().reset_index().sort_values("PREJUIZO_ESTIMADO", ascending=False).head(7)
        fig = px.bar(dmg, x="CLIENTE", y="PREJUIZO_ESTIMADO", title="Impacto Financeiro", color_discrete_sequence=["#ef4444"])
        fig.update_layout(**L)
        st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    st.caption("Ataques em tempo real direcionados ao Brasil")
    ...
    COORDS = {
        ...
        "Canada": (56.13, -106.34), "South Korea": (35.9, 127.76)
    }
    TARGET = (-15.78, -47.92)
    ...
    <head><meta charset="utf-8"><style>
        ...
        #info{{position:absolute;bottom:15px;left:15px;background:rgba(0,0,0,0.7);border:1px solid #00d4ff;border-radius:8px;padding:8px 15px;color:#00d4ff;font-size:12px;z-index:100;}}
    </style></head>
    <body>
    <canvas id="c"></canvas>
    ...
    <script>
        ...
        function drawCountries() {{
            for(var c of countries) {{
                ...
                ctx.beginPath();
                ctx.arc(p[0], p[1], isTarget ? 8 : 3, 0, Math.PI * 2);
                ctx.fillStyle = isTarget ? '#00ff00' : 'rgba(0,200,255,0.5)';
                ctx.fill();
                ctx.fillStyle = isTarget ? '#00ff00' : 'rgba(0,200,255,0.8)';
                ctx.font = isTarget ? 'bold 10px monospace' : '8px monospace';
        ...
        function spawn() {{
            if (arcs.length && Math.random() < 0.1) {{
...
with tab4:
    ...
    with st.form("chat_form", clear_on_submit=True):
        pergunta = st.text_input("", placeholder="Digite sua pergunta...", label_visibility="collapsed")
    ...
    if enviar and pergunta.strip():
        if GEMINI_API_KEY:
            ...
            with st.spinner("🤔 Pensando..."):
                try:
                    prompt = f"""Você é um especialista em segurança cibernética. Responda em português de forma profissional.

Contexto:
- Total de incidentes: {len(df_vis)}
- Críticos: {incidentes_criticos}
- IPs bloqueados: {ips_bloqueados}
- Prejuízo: R$ {prejuizo_total:,.0f}
...
with tab6:
    ...
    if "logs_sistema" in st.session_state:
        for log in reversed(st.session_state["logs_sistema"][-20:]):
...
<div style="text-align:center;padding:1rem 0;border-top:1px solid rgba(0,200,255,0.05);margin-top:1rem;">
  <p style="color:#152535;font-size:.6rem;">SENTINELAI — PLATAFORMA DE SEGURANÇA CIBERNÉTICA | LGPD COMPLIANT</p>
</div>
""", unsafe_allow_html=True)
